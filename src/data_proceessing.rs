...
pub fn json_to_polars(auction_items: &[AuctionItem]) -> Result<DataFrame, Box<dyn std::error::Error>> {
  let uuid: Vec<&str> = auction_items.iter().map(|item| item.uuid()).collect();
  let item_name: Vec<&str> = auction_items.iter().map(|item| item.item_name()).collect();
  let item_lore: Vec<&str> = auction_items.iter().map(|item| item.item_lore()).collect();
  let price: Vec<u64> = auction_items.iter().map(|item| item.price()).collect();
